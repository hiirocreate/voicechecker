...
- 端末が対応していれば `NoiseSuppressor`(定常的な背景ノイズの軽減)を自動的に有効化します。
- 自動音量調整(AGC)はあえて有効化していません。AGCは音量を常に均一に近づけてしまうため、
  「音域ごとの声量プロファイル(得意/苦手音域の判定)」の精度を損なってしまうためです。
- 画面上の音量メーターは、細かい揺れを抑えるためVUメーター風の平滑化(ballistics)と
  アニメーション補間を行っています。ただし声域測定・ビブラート解析に使う内部の数値は
  平滑化前の生データのままなので、表示の見やすさと分析精度を両立しています。
